...
+function ($) {
    ...
    var defaults = {
        ...
        pushAnimationDuration: 400  //ä¸è¦åŠ¨è¿™ä¸ªï¼Œè¿™æ˜¯è§£å†³å®‰å“ animationEnd äº‹ä»¶æ— æ³•è§¦å‘çš„bug
    };
...
;(function () {
    ...
    FastClick.prototype.onTouchEnd = function(event) {
        ...
        //ä¿®å¤å®‰å“å¾®ä¿¡ä¸‹ï¼Œinput type="date" çš„bugï¼Œç»æµ‹è¯•date,time,monthå·²æ²¡é—®é¢˜
        var unsupportedType = ['date', 'time', 'month'];
        if(unsupportedType.indexOf(event.target.type) !== -1){
            ã€€ã€€ã€€ã€€return false;
            ã€€ã€€}
    ...
    var Scroller = function(pageContent, _options) {
        ...
        if (useJSScroller) {
            ...
            var options = {
                probeType: 1,
                mouseWheel: true,
                //è§£å†³å®‰å“jsæ¨¡å¼ä¸‹ï¼Œåˆ·æ–°æ»šåŠ¨æ¡åŽç»‘å®šçš„äº‹ä»¶ä¸å“åº”
                click:true
            };
            if (ptr) {
                options.ptr = true;
...
+function($) {
    ...
    var Router = function() {
        ...
        this.stack.setItem("back", "[]");  //è¿”å›žæ ˆ, {url, pageid, stateid}
        this.stack.setItem("forward", "[]");  //å‰è¿›æ ˆ, {url, pageid, stateid}
        this.init();
        this.xhr = null;
        // è§£å†³å„ä¸ªwebviewé’ˆå¯¹é¡µé¢é‡æ–°åŠ è½½ï¼ˆåŒ…æ‹¬åŽé€€é€ æˆçš„ï¼‰æ—¶History Stateçš„å¤„ç†å·®å¼‚ï¼ŒåŠ æ­¤æ ‡å¿—ä½
        this.newLoaded = true;
    };
    ...
    Router.prototype.init = function() {
        ...
        // ç¡®ä¿æ˜¯ page æ—¶æ‰åˆ‡æ¢æ˜¾ç¤ºï¼Œä¸ç„¶å¯èƒ½åªæ˜¯æ™®é€šçš„ hashï¼ˆ#129ï¼‰
        ...
        // éœ€è¦è®¾ç½®å…¥å£é¡µçš„Urlï¼Œæ–¹ä¾¿ç”¨æˆ·åœ¨ç±»ä¼¼xx/yy#step2 çš„é¡µé¢åˆ·æ–°åŠ è½½åŽ ç‚¹å‡»åŽé€€å¯ä»¥å›žåˆ°å…¥å£é¡µ
            entryUrl = curUrl.split('#')[0];

        // åœ¨é¡µé¢åŠ è½½æ—¶ï¼Œå¯èƒ½ä¼šåŒ…å«ä¸€ä¸ªéžç©ºçš„çŠ¶æ€å¯¹è±¡history.stateã€‚è¿™ç§æƒ…å†µæ˜¯ä¼šå‘ç”Ÿçš„ï¼Œä¾‹å¦‚ï¼Œå¦‚æžœé¡µé¢ä¸­ä½¿ç”¨pushState()æˆ–replaceState()æ–¹æ³•è®¾ç½®äº†ä¸€ä¸ªçŠ¶æ€å¯¹è±¡ï¼Œç„¶åŽç”¨æˆ·é‡å¯äº†æµè§ˆå™¨ã€‚https://developer.mozilla.org/en-US/docs/Web/API/History_API#Reading_the_current_state
    ...
    //åŠ è½½ä¸€ä¸ªé¡µé¢,ä¼ å…¥çš„å‚æ•°æ˜¯é¡µé¢idæˆ–è€…url
    Router.prototype.loadPage = function(url) {

        // android chrome åœ¨ç§»åŠ¨ç«¯åŠ è½½é¡µé¢æ—¶ä¸ä¼šè§¦å‘ä¸€æ¬¡â€˜popstateâ€™äº‹ä»¶
     ...
     * é¡µé¢è½¬åœºæ•ˆæžœ
     *
     * é¦–å…ˆç»™è¦ç§»å…¥å±•ç¤ºçš„é¡µé¢æ·»åŠ ä¸Šå½“å‰é¡µé¢æ ‡è¯†ï¼ˆpage-currentï¼‰ï¼Œè¦ç§»å‡ºå±•ç¤ºçš„ç§»é™¤å½“å‰é¡µé¢æ ‡è¯†ï¼›
     * ç„¶åŽç»™ç§»å…¥ç§»é™¤çš„é¡µé¢æ·»åŠ ä¸Šå¯¹åº”çš„åŠ¨ç”» classï¼ŒåŠ¨ç”»ç»“æŸåŽæ¸…é™¤åŠ¨ç”» class å¹¶å‘é€å¯¹åº”äº‹ä»¶ã€‚
     *
     * æ³¨æ„ï¼Œä¸èƒ½åœ¨åŠ¨ç”»åŽæ‰ç»™ç§»å…¥å±•ç¤ºçš„é¡µé¢æ·»åŠ å½“å‰é¡µé¢æ ‡è¯†ï¼Œå¦åˆ™ï¼Œåœ¨å¿«é€Ÿåˆ‡æ¢çš„æ—¶å€™å°†ä¼šå› ä¸ºæ²¡æœ‰ .page-current
     * çš„é¡µé¢è€ŒæŠ¥é”™ï¼ˆå…·ä½“æ¥è¯´æ˜¯æ‰¾è¿™ç±»é¡µé¢çš„ id æ—¶æŠ¥é”™ï¼Œç›®å‰å¹¶æ²¡æœ‰ç¡®ä¿ id æŸ¥æ‰¾çš„å¥å£®æ€§ï¼‰
     *
     * @param leftPage ä»Žæ•ˆæžœä¸Šçœ‹ä½äºŽå·¦ä¾§çš„é¡µé¢ï¼ŒjQuery/Zepto å¯¹è±¡
     * @param rightPage ä»Žæ•ˆæžœä¸Šä½äºŽå³ä¾§çš„é¡µé¢ï¼ŒjQuery/Zepto å¯¹è±¡
     * @param {Boolean} leftToRight æ˜¯å¦æ˜¯ä»Žå·¦å¾€å³åˆ‡æ¢ï¼ˆä»£è¡¨æ˜¯åŽé€€ï¼‰ï¼Œé»˜è®¤æ˜¯ç›¸å½“äºŽ false
     */
    ...
    // å…¶å®žæ²¡è°ƒç”¨åˆ°ï¼Œå¦‚æžœæ— æ³•å‰è¿›ï¼Œåˆ™åŠ è½½å¯¹åº”çš„url
    ...
    // ç‚¹å‡» .back æŒ‰é’®ï¼Œå¦‚æžœæ— æ³•åŽé€€ï¼Œåˆ™åŠ è½½å¯¹åº”çš„url
    ...
    Router.prototype.onpopstate = function(d) {
        var state = d.state;
        // åˆ·æ–°å†åŽé€€å¯¼è‡´æ— æ³•å–åˆ°state
    ...
    //æ ¹æ®urlèŽ·å–é¡µé¢çš„DOMï¼Œå¦‚æžœæ˜¯ä¸€ä¸ªå†…è”é¡µé¢ï¼Œåˆ™ç›´æŽ¥è¿”å›žï¼Œå¦åˆ™ç”¨ajaxåŠ è½½
